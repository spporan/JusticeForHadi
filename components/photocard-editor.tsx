...
<html>
  ...
  const openImageForManualSaveForFBIOS = (
    win: Window,
    blob: Blob
  ): void => {
    const reader = new FileReader();
...
">
  ...
  const handleExport = () => {
    // 🔐 iOS Facebook → OPEN WINDOW FIRST
    let iosFBWindow: Window | null = null;

    if (isFacebookInAppIOSBrowser()) {
      iosFBWindow = window.open('', '_blank');
      if (!iosFBWindow) return;
    }

    // ⚠️ Now do async work
    exportCanvas(selectedImage, textLayers, startDate, timeHeader)
      .then((blob) => {
        if (!blob) return;
